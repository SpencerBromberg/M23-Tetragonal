print "M23_W_COORDINATE_PROBE_BEGIN";
p := 1013;
print "PRIME =", p;
k := GF(p);
...
// Fixed-factor probe for the two adjoint representatives.
// Convert the degree<23 u-expansions to a genuine bivariate polynomial ring
// over F_p so that any common polynomial factor becomes visible.
P2<T,U> := PolynomialRing(k,2);

function ToBivariate(H)
    hs := ElementToSequence(H);
    HH := P2!0;
    for i in [1..23] do
        rr := hs[i];
        dd := A!Denominator(rr);
        if Degree(dd) ne 0 then
            error "adjoint representative is not polynomial";
        end if;
        nn := A!Numerator(rr) / LeadingCoefficient(dd);
        for a in [0..Degree(nn)] do
            HH +:= (k!Coefficient(nn,a))*T^a*U^(i-1);
        end for;
    end for;
    return HH;
end function;

function WeightedDegree42(F)
    if F eq 0 then
        return -1;
    end if;
    mons := Monomials(F);
...
print "ADJOINT_BIVARIATE_DEG_U =", Degree(H1,2), Degree(H2,2);
print "ADJOINT_WEIGHTED_DEGREES =", WeightedDegree42(H1), WeightedDegree42(H2);
print "ADJOINT_TERM_COUNTS =", #Terms(H1), #Terms(H2);

print "CHECK FIXED FACTOR START";
Hgcd := GCD(H1,H2);
if Hgcd ne 0 then
    Hgcd /:= LeadingCoefficient(Hgcd);
end if;
print "ADJOINT_GCD_TOTAL_DEGREE =", TotalDegree(Hgcd);
print "ADJOINT_GCD_DEG_T =", Degree(Hgcd,1);
print "ADJOINT_GCD_DEG_U =", Degree(Hgcd,2);
print "ADJOINT_GCD_WEIGHTED_DEGREE =", WeightedDegree42(Hgcd);
print "ADJOINT_GCD_TERM_COUNT =", #Terms(Hgcd);

Q1 := ExactQuotient(H1,Hgcd);
Q2 := ExactQuotient(H2,Hgcd);
print "MOVING_Q1_TOTAL_DEGREE =", TotalDegree(Q1);
print "MOVING_Q2_TOTAL_DEGREE =", TotalDegree(Q2);
print "MOVING_Q1_DEG_T_U =", Degree(Q1,1), Degree(Q1,2);
print "MOVING_Q2_DEG_T_U =", Degree(Q2,1), Degree(Q2,2);
print "MOVING_Q1_WEIGHTED_DEGREE =", WeightedDegree42(Q1);
print "MOVING_Q2_WEIGHTED_DEGREE =", WeightedDegree42(Q2);
print "MOVING_Q1_TERM_COUNT =", #Terms(Q1);
print "MOVING_Q2_TERM_COUNT =", #Terms(Q2);
print "ADJOINT_FIXED_FACTOR_PROBE_PASS";
